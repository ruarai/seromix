...
plot_model_data <- function(model_data, run_name, plot_individuals = FALSE) {
  observations <- model_data$observations %>%
    mutate(strain_year = model_data$modelled_years[ix_strain],
           year_sampled = model_data$modelled_years[ix_t_obs])
  ...
  if(plot_individuals) {
    ...
    for(i in 1:n_pages) {
      p <- observations %>%
        ...
        facet_grid(rows = vars(year_sampled), cols = vars(ix_subject)) +

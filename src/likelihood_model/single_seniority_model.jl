...
@model function waning_model_single_seniority(
    ...
    log_likelihood = general_waning_likelihood(
        ...
        observed_titre,
        model_cache,
        individual_waning_single_seniority!,
...
function individual_waning_single_seniority!(
    params,
    ...
    for ix_t in max(1, subject_birth_ix):n_t_steps
        ...
        # Effect only changes between zero and one prior infections
        prior_infections = min(prior_infections + 1, 1.0)
...
turing_function_to_waning_function(model_f::typeof(waning_model_single_seniority)) = individual_waning_single_seniority!

...
@model function waning_model_linear(
    ...
    mu_long ~ LogNormal(0.0, 1.0)
    mu_short ~ LogNormal(0.0, 1.0)

    dist_scale ~ LogNormal(0.0, 1.0)

    r_waning ~ LogNormal(0.0, 1.0)

    obs_sd ~ LogNormal(0.0, 1.0)
    ...
    for ix_subject in subjects_to_process
        ...
        waning_curve_individual_linear!(
            mu_long, mu_short, dist_scale, r_waning,

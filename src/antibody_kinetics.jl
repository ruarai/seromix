...
function waning_curve_individual_in_place_3!(
    mu_long::T,
    mu_short::T, omega::T,
    sigma_long::T, sigma_short::T,
    tau::T,
    ...
    obs_y::Vector{Float64}, obs_sd::T, obs_min::T, obs_max::T
) where T <: Real
    logprob = 0.0
    for (ix_t_obs, obs_at_t_obs) in obs_lookup_ind
        for (ix_obs_strain, ix_obs) in obs_at_t_obs
            y_val = 0.0

            prior_infections = 0.0
            for ix_t in max(1, subject_birth_ix):ix_t_obs
                @inbounds if !infections[ix_t]
                    continue
                end
                ...
                y_val += seniority * (long_term + short_term)

                prior_infections += 1.0
            end

            # logprob += logpdf(TitreNormal(y_val, obs_sd, obs_min, obs_max), obs_y[ix_obs])
            logprob += titre_logpdf_component(obs_y[ix_obs], y_val, obs_sd, obs_min, obs_max)
        end
    end
